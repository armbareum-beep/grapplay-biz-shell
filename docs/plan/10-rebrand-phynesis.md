...
| D2 | 브랜드 마크 | ✅ 확정(2026-09-19 변경) | **PH 모노그램 마크** 하나로 통일 — 화면(헤더/푸터/로그인)·파비콘·OG 이미지 전부 같은 마크. 공통 `BrandLogo.tsx`(패스 인라인, `currentColor`). 자산 `public/logo/mark*.svg`, 재생성 `scripts/trace-logo.py` + `scripts/build-brand-assets.py`. ~~2줄 텍스트 워드마크(파이네시스 / PHYNESIS, SUIT Bold 서브셋)~~ 와 ~~PNS 심볼~~ 은 대체돼 삭제 |
...
**브랜드 마크 교체 — 텍스트 워드마크 → PH 모노그램 (2026-09-19, D2 변경)**
- 워드마크(파이네시스 / PHYNESIS 2줄)를 쓰지 않고, 제공받은 **PH 모노그램 로고**를 브랜드 마크로 삼는다. 헤더·푸터·로그인·파비콘·OG 이미지 전부 같은 마크.
- 받은 파일이 알파 없는 4096×4096 래스터라 **직접 벡터로 추적**했다(`scripts/trace-logo.py`). 픽셀 경계를 따라 폐곡선을 만들고 Douglas-Peucker로 단순화 — 계수 1.2에서 원본과 시각적으로 동일하면서 1.4KB. 결과 viewBox 670×700.
  - 추적기에서 두 번 걸렸다: ① 폐곡선은 시작·끝점이 같아 DP가 퇴화하므로 가장 먼 점으로 둘로 잘라 각각 단순화해야 한다. ② 원본을 정사각형으로 강제 리사이즈하면 마크가 찌그러진다(긴 변 기준으로 비율 유지).
- `BrandLogo.tsx`는 패스를 인라인으로 들고 `currentColor`를 따른다 — 밝은/어두운 배경에서 색만 바뀐다. 크기는 sm 26 / md 32 / lg 52px. 정사각에 가까운 마크라 2줄 텍스트 워드마크와 같은 존재감을 내려면 높이를 더 키워야 했다.
- 파비콘 여백은 크기별로 다르게 뒀다(16px 6% → 512px 20%). 작은 탭 아이콘에 같은 여백을 쓰면 형태가 안 보인다.
- 남은 판단: 헤더에 이름 문자열이 없어, 처음 온 사람은 히어로·철학 섹션까지 내려가야 "파이네시스"를 본다. 인지도가 붙기 전까지는 마크 옆에 이름을 병기하는 편이 나을 수 있다(미적용).
  → 일부 해소(2026-09-19): 철학 섹션의 합성 식에 마크와 "파이네시스"를 나란히 놓아, 스크롤 중 한 번은 이름과 마크를 같이 보게 된다.

**히어로·철학 섹션 업그레이드 + 리브랜딩 잔여 정리 (2026-09-19)**

히어로:
- **숫자(6 주제 / 3 강의 / 2 전문가) 제거.** 숫자는 규모가 무기일 때 쓰는 장치다. 강의 3개·전문가 2명을 대문에 내걸면 신뢰를 주는 게 아니라 "아직 비어 있는 사이트"라고 광고한다. 대신 **전문가 얼굴·이름·직함**을 놓아 "누가 가르치는가"로 신뢰를 만든다. 단 **실제 사진이 있는 전문가만** — 이모지 폴백 얼굴은 오히려 미완성으로 보여, 사진이 없으면 줄 자체를 숨긴다.
- **CTA의 로그인 벽 제거.** `session ? '/content?free=1' : '/auth'` 라 비로그인 사용자는 첫 화면 버튼에서 바로 로그인을 만났다. 목적지를 항상 `/content?free=1`로 두고 문구도 "무료 강의 먼저 보기"로, 아래에 "가입 없이 둘러볼 수 있습니다"를 덧붙였다. 결제·수강 시점에 각 상세 화면이 알아서 로그인을 요구한다.
- **높이 축소**(`pb-20 pt-16` → `pb-14 pt-12`). 첫 화면이 히어로만으로 끝나 스크롤 유인이 없었다. 지금은 390×844에서 다음 섹션이 y=466부터 걸쳐, 첫 질문이 화면 안에 들어온다.
- **한글에 음수 자간을 쓰지 않는다.** 헤드라인의 `tracking-tight`(-0.025em)를 뺐다. 한글 음절은 이미 좌우 여백이 균형 잡혀 있어 음수 자간을 주면 글자 사이가 불규칙해 보인다. 행간도 `leading-[1.1]` → `1.18`(한글은 음절 블록이 세로로 꽉 차 1.1이 좁다).
  - 좌측 정렬은 실측 결과 문제가 아니었다: 박스는 전부 16px로 맞고, 잉크 기준 차이는 헤드라인 1행 17.25px / 2행 16.50px로 0.75px. 첫 글자(실/사)의 좌측 베어링 차이라 코드로 맞출 성질이 아니다. **실제 어긋나 보인 건 숫자 줄**이었다 — `border-l pl-4` 때문에 혼자 17px 안쪽에서 시작했고, 그 줄을 없애면서 해소됐다.

철학 섹션:
- **합성어임을 글자로 드러낸다.** 한국어 표기에서 어원이 그대로 맞아떨어진다: **파이**(데이아) + (프로)**네시스** = **파이네시스**. 두 낱말에서 가져온 음절만 흰색으로 두고 나머지를 죽인 뒤, 아래에 `파이 + 네시스 = [마크] 파이네시스` 식을 놓았다. 설명 문장 없이 형태만으로 읽힌다.
- 맺음 문구를 "우리는 이 둘을 사업에 연결합니다" → "배움과 실천적 지혜를 사업에 연결합니다"로. 앞의 식이 이미 "이 둘"을 보여주므로 지시어 대신 내용을 쓴다.
- 하단의 텍스트 워드마크(SUIT `PHYNESIS 파이네시스`)를 없앴다 — 헤더는 마크인데 여기만 텍스트라 브랜드 표현이 두 벌이었다.
- 2열 설명에 `min-h`를 줘 모바일에서 두 열의 끝 줄을 맞췄다(왼쪽 1줄 / 오른쪽 2줄 비대칭 해소).
- 섹션 말미에 `/experts` 링크 추가. 어원 설명에 공감한 사람이 갈 곳이 없었다.

리브랜딩 잔여(스크린샷에서 발견):
- 상단 메뉴줄의 전문가·관리자 대시보드 링크는 **그대로 둔다**(한 번 뺐다가 되돌림). 로그인한 전문가·관리자에게만 붙는 역할 메뉴라 공개 노출이 아니고, 메뉴줄은 가로 스크롤이라 넘쳐도 잘리는 게 아니라 밀린다. 드롭다운과 중복이지만 자주 쓰는 진입점이라 둘 다 두는 편이 낫다는 판단.
- **"지도자 대시보드" → "전문가 대시보드".** 리브랜딩 때 "전문가"로 통일했는데 이 메뉴만 옛 용어가 남아 있었다. (전문가 대시보드 화면 내부와 약관의 "지도자" 표기는 아직 남음 — 별도 정리 필요)
- **내 강의(`/my`) 썸네일이 표지가 아니라 이모지였다.** 옛 `c.cover`(밝은 emerald·rose 그라데이션) + `c.thumbEmoji`를 쓰고 있어 리브랜딩된 카드와 완전히 달랐다. `coverImage` 우선 + 없으면 `COVER_BY_CATEGORY` 다크 그라데이션으로, `CourseCard`와 같은 규칙으로 맞췄다.
- 같은 화면의 **앰버 잔재 정리**: 탭 밑줄·진행바·아바타 폴백 배경·프로필 저장 버튼·입력 포커스 테두리를 `brand-*`로. `ExpertAvatar`의 기본 `fallbackBg`도 `bg-amber-100` → `bg-slate-100`.
- 남은 과제: 전문가 아바타가 사진 없을 때 **이모지로 폴백**한다(전 화면 19곳). 리브랜딩에서 이모지를 걷어낸 원칙과 어긋나므로 선 아이콘이나 이니셜 폴백으로 바꿔야 하지만, 관리자 화면이 같은 컴포넌트를 콘텐츠 썸네일 용도로도 쓰고 있어 별도 작업으로 둔다.
