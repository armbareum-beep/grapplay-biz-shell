...
- 히어로 맨 위 영문 라벨을 `BUSINESS EDUCATION FOR PROFESSIONALS` → **`BUSINESS EDUCATION`** 으로 줄임. (한 번 라벨 전체를 지웠다가 정정 — 지울 대상은 `FOR PROFESSIONALS` 뿐이었다. 대상이 "모든 전문가"라는 건 이미 브랜드 정의에 있고, 라벨에서까지 반복할 필요가 없다는 판단.)
- 히어로 하단 `PAIDEIA × PHRONESIS — 배움과 실천적 지혜를 사업으로 연결합니다` 줄 삭제. 어원 설명은 아래 철학 섹션이 2열로 제대로 하고 있어 히어로에서 미리 꺼낼 이유가 없다. 철학 섹션의 PAIDEIA/PHRONESIS 블록은 그대로 유지.
...
**히어로 배경 워터마크 크기 (2026-09-19)**
- 모바일에서 배경 워터마크 `PHYNESIS`의 앞글자가 잘려 `NESIS`만 보이던 문제. 글자 크기가 `140px` 고정이라 렌더 폭이 719px(≈5.13em)인데 워터마크는 오른쪽(`-right-4`)에 붙어 있어, 360~430px 화면에서는 왼쪽 절반이 섹션의 `overflow-hidden`에 잘려나갔다.
- 화면 폭에 비례하는 `text-[18vw]`로 교체(컨테이너가 `max-w-6xl`로 고정되는 `xl` 이상만 `220px` 고정). 320px~1920px 전 구간에서 8글자가 모두 화면 안에 들어오는 것을 실측 확인.
- 하단 오프셋도 `-bottom-10`(40px 고정) → `-bottom-[0.28em]`. 고정 px이면 글자가 작아진 모바일에서 세로로 절반 넘게 잘린다. `em`으로 두면 어느 크기에서나 같은 비율로만 걸친다.
