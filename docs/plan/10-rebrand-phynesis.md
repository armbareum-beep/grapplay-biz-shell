...
4. ~~2차 액센트 골드~~ → **철회(2026-09-19)**: 골드 글씨 전부 제거. 라벨·번호는 slate-400, 다크 섹션 라벨은 brand-300, 무료 표시는 brand-600, 별점은 amber-400 → 이후 **완전 제거(2026-09-19)**: 색을 다른 색으로 바꾸지 않고 클래스 자체를 없애 부모 요소 색을 그대로 상속(별점 전용 색 없음). 포인트 컬러는 네이비 하나.
...
- 05 전문가(신뢰): DB experts — 사진·직함·약력 2줄·분야. 06 후기 마퀴.
